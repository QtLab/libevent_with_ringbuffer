...
int xxsocket::connect_n(socket_native_type s, const ip::endpoint_v4& ep, timeval* timeout)
{
    ...
    if ((n = xxsocket::connect(s, ep)) < 0) {
        ...
        if (!err_connect_retriable(error)) //(error != EINPROGRESS && error != EWOULDBLOCK)
            return -1;
    }
...
int xxsocket::send_n(socket_native_type s, const void* buf, int len, timeval* timeout, int flags)
{
        ...
        bytes_transferred += n)
    {
        ...
        if (n <= 0)
        {
            ...
            errcode = errno;
#endif
            if (n == -1 && err_rw_retriable(errcode))
            {
...
int xxsocket::send(void* buf, int len, int slicelen, int flags) const
{
    ...
    int sent_bytes = 1;
    while (bytes_leavings > 0 && sent_bytes > 0)
    {
        if (bytes_leavings > slicelen)
        {
            sent_bytes = this->send(ptr, slicelen, flags);
            ptr += slicelen;
        }
        else
        {
            sent_bytes = this->send(ptr, bytes_leavings, flags);
        }
        bytes_leavings -= sent_bytes;
    }
...
int xxsocket::recv(void* buf, int len, int slicelen, int flags) const
{
    ...
    int bytes_leavings = len;
    int bytes_recv = 1;
    while(bytes_leavings > 0 && bytes_recv > 0)
    {
        if(bytes_leavings > slicelen)
        {
            bytes_recv = this->recv_i(ptr, slicelen, flags);
            ptr += slicelen;
        }
        else
        {
            bytes_recv = this->recv_i(ptr, bytes_leavings, flags);
        }
        bytes_leavings -= bytes_recv;
    }
    return len - bytes_leavings;
}
